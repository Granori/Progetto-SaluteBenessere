...
export default function Titolo() {
    const [dati, setDati] = useState([]); // Stato per salvare i dati
    const [caricamento, setCaricamento] = useState(true); // Stato per il caricamento

    useEffect(() => {
        // La funzione fetch parte appena il componente viene mostrato
        fetch('/api/test')
            .then(response => response.json())
            .then(data => {
                setDati(data.messaggio); // Salviamo i dati nello stato
                setCaricamento(false); // Fine caricamento
            })
            .catch(error => console.error("Errore:", error));
    }, []); // [] significa: esegui solo una volta all'avvio
        ...
        <div class="min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            <header class="w-full bg-white border-b border-slate-200 px-10 py-5 flex justify-between items-center shadow-sm">
                ...
                <nav class="flex gap-8 text-sm font-bold uppercase tracking-widest text-slate-500">
                    <a href="#" class="hover:text-emerald-500 transition">Home</a>
                    <a href="#" class="text-emerald-600 border-b-2 border-emerald-600">Calcolatore BMI</a>
                    <a href="#" class="hover:text-emerald-500 transition">Conta Calorie</a>
                </nav>
            </header>
